...
public:
	...
	void setPosX(float nPosX) {
		this->posX = nPosX;
	}

	void setPosZ(float nPosZ) {
		this->posZ = nPosZ;
	}
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale, bool movCam, bool cameraType)
	{
		...
		//Ajuste a la camara
		D3DXMATRIX translationFirstPerson;
		if (cameraType) {
			D3DXMatrixTranslation(&translationFirstPerson, -0.5, 0.0, 1.2);
		}
		else {
			D3DXMatrixTranslation(&translationFirstPerson, -0.5, 0.0, 12.0);
		}
		...
		//Seteo la matriz de mundo con la transformación en primera persona
		D3DXMATRIX worldMat = rotationMat * scaleMat * translationMat;
		if (movCam) {
			worldMat = scaleMat * translationFirstPerson * rotationMat * translationMat;
		}

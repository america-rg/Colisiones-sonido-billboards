...
public:	
	...
	Camara *camara;
	//ModeloRR* model;
	////aqui los modelos que voy a usar
	//ModeloRR* equis;
	//ModeloRR* parada;
	//ModeloRR* cono;
	//ModeloRR* zom;
	...
	//Angulo de rotacion de la camara y tipo de camara
	float rotCam;
	bool cameraType;
    ...
    DXRR(HWND hWnd, int Ancho, int Alto)
	{
		...
		skydome = new SkyDome(32, 32, 100.0f, &d3dDevice, &d3dContext, L"sky.png");

		//BILLBOARDS
		billboard = new BillboardRR(L"Assets/Billboards/fuego-anim.png",L"Assets/Billboards/fuego-anim-normal.png", d3dDevice, d3dContext, 5);
		tree = new BillboardRR(L"Assets/Billboards/Tree.png", L"Assets/Billboards/Tree_normalMap.png", d3dDevice, d3dContext, 5);

		//nombre del modelo, el obj, la textura 2 veces, posicion
		/*model = new ModeloRR(d3dDevice, d3dContext, "Assets/Cofre/Cofre.obj", L"Assets/Cofre/Cofre-color.png", L"Assets/Cofre/Cofre-spec.png", 0, 0);
		equis = new ModeloRR(d3dDevice, d3dContext, "Assets/aud/ref1.obj", L"Assets/aud/tex1.png", L"Assets/aud/tex1.png",10,15);
		parada = new ModeloRR(d3dDevice, d3dContext, "Assets/Stop sign/Stop sign.obj", L"Assets/Stop sign/diss_00.png", L"Assets/Stop sign/diss_00.png", 0, 17);
		cono = new ModeloRR(d3dDevice, d3dContext, "Assets/Traffic Cone/m_TrafficCone01.obj", L"Assets/Traffic Cone/mat_TrafficCone01_Albedo.png", L"Assets/Traffic Cone/mat_TrafficCone01_Albedo.png", 0, 13);
		zom = new ModeloRR(d3dDevice, d3dContext, "Assets/zom/zombie/zombie.obj", L"Assets/zom/zombie/zombie.png", L"Assets/zom/zombie/zombie.png", 0, 25);
		perro = new ModeloRR(d3dDevice, d3dContext, "Assets/Dog/Dog/Dog.obj", L"Assets/Dog/Dog/dog_white.png", L"Assets/Dog/Dog/dog_white.png",0, 16);*/

		//ACTORES
		jeep = new ModeloRR(d3dDevice, d3dContext, "Assets/Jeep/Jeep.obj", L"Assets/Jeep/buggy001.png", L"Assets/Jeep/buggy001.png", 0, 0);

		//OBJETOS DEL ENTORNO
		barricade = new ModeloRR(d3dDevice, d3dContext, "Assets/Barricade/Barricade.obj", L"Assets/Barricade/op102_obj03.png", L"Assets/noSpecMap.jpg", 20, 0);
		rock = new ModeloRR(d3dDevice, d3dContext, "Assets/Rock/Rock.obj", L"Assets/Rock/enemy_rock2_s3tc.png", L"Assets/noSpecMap.jpg", -20, 0);
		tankGas = new ModeloRR(d3dDevice, d3dContext, "Assets/Tank gas/Tank gas.obj", L"Assets/Tank gas/Tank.png", L"Assets/Tank gas/Tank.png", 40, 0);
	...
	bool IniciaD3D(HWND hWnd)
	{
		...
		res = m_XACT3.LoadWaveBank(L"Assets/Sonido/Win/Wave Bank.xwb");
		if (!res) {
			return false;
		}
		res = m_XACT3.LoadSoundBank(L"Assets/Sonido/Win/Sound Bank.xsb");
		if (!res) {
			return false;
		}

		//REPRODUCIR
		cueIndex = m_XACT3.m_pSoundBank->GetCueIndex("Soundtrack");
		m_XACT3.m_pSoundBank->Play(cueIndex, 0, 0, 0);
		cueIndex = m_XACT3.m_pSoundBank->GetCueIndex("Hit");
	...
	void Render(void)
	{
		...
		//Seteamos la posicion de ambas camaras 1a y 3a persona
		...
		if (gas <= 0.0) {
			camara->UpdateCam(0, arriaba, izqder, cameraType);
		}
		else {
			camara->UpdateCam(vel, arriaba, izqder, cameraType);
		}
		...
		//COLISIONES
		//ROCKS
		if (isPointInsideSphere(camara->getPos(), rock->getSphere(7))) {
		...
		//TANKGAS
		if (isPointInsideSphere(camara->getPos(), tankGas->getSphere(5))) {
			gas += 20.0f;
			tank1pick = true;
		}
		...
		tree->Draw(camara->vista, camara->proyeccion, camara->posCam,
			11, -78, terreno->Superficie(11,-78), 5, uv1, uv2, uv3, uv4, frameBillboard, false);

		//TurnOffAlphaBlending();
		//se dibuja el modelo, la vista que tendrá y el ultimo es la escala
		/*model->Draw(camara->vista, camara->proyeccion, terreno->Superficie(100, 20), camara->posCam, 10.0f, 0, 'A', 1);
		equis->Draw(camara->vista, camara->proyeccion, terreno->Superficie(100, 100), camara->posCam, 10.0f, 0, 'B', 0.04);
		parada->Draw(camara->vista, camara->proyeccion, terreno->Superficie(100, 20), camara->posCam, 10.0f, 0, 'C', 2);
		cono->Draw(camara->vista, camara->proyeccion, terreno->Superficie(100, 20), camara->posCam, 10.0f, 0, 'E', 0.7);
		zom->Draw(camara->vista, camara->proyeccion, terreno->Superficie(100, 20), camara->posCam, 10.0f, 0, 'F', 0.85);
		perro->Draw(camara->vista, camara->proyeccion, terreno->Superficie(100, 20), camara->posCam, 10.0f, 0, 'G', 0.5);*/

		//MODELOS
		//Seteamos la posicion del carro a la posicion de la camara y lo dibujamos
		jeep->setPosX(camara->hdveo.x);
		jeep->setPosZ(camara->hdveo.z);
		jeep->Draw(camara->vista, camara->proyeccion, terreno->Superficie(jeep->getPosX(), jeep->getPosZ()) + 0.5, camara->posCam, 10.0f, XM_PI + rotCam, 'Y', 0.8, true, cameraType);
		barricade->Draw(camara->vista, camara->proyeccion, terreno->Superficie(0, 0), camara->posCam, 10.0f, 0, 'A', 1, false, cameraType);
		rock->Draw(camara->vista, camara->proyeccion, terreno->Superficie(0, 0), camara->posCam, 10.0f, 0, 'A', 1, false, cameraType);
		if (!tank1pick) {
			tankGas->Draw(camara->vista, camara->proyeccion, terreno->Superficie(0, 0), camara->posCam, 10.0f, 0, 'A', 1, false, cameraType);
		}
		else {
